...
export class AdminService {
  ...
  async getOwners(page = 1, limit = 20, search = '') {
    ...
    let query = supabase
      .from('users')
      .select(
        'id, email, first_name, last_name, phone_number, created_at, status',
        { count: 'exact' },
      )
      .eq('role', 'owner')
      .order('created_at', { ascending: false })
    ...
    if (search) {
      query = query.or(
        `email.ilike.%${search}%,first_name.ilike.%${search}%,last_name.ilike.%${search}%,business_name.ilike.%${search}%`,
      );
    }
    ...
    const { data: accounts } = ownerIds.length
      ? await supabase
          .from('owner_accounts')
          .select(
            'primary_owner_id, business_name, subscription_status, trial_ends_at, stripe_customer_id',
          )
  ...
  async getEvents(page = 1, limit = 20, search = '', ownerId = '') {
    ...
    let query = supabase
      .from('event')
      .select(
        'id, name, description, date, start_time, end_time, venue, location, guest_count, status, owner_id, budget, created_at',
        { count: 'exact' },
      )
      .order('created_at', { ascending: false })
      .range(offset, offset + limit - 1);
    ...
    const ownerIds = [
      ...new Set((data || []).map((e: any) => e.owner_id).filter(Boolean)),
    ];
  ...
  async getBookings(page = 1, limit = 20, search = '', ownerId = '') {
    ...
    if (search) {
      query = query.or(
        `contact_name.ilike.%${search}%,contact_email.ilike.%${search}%`,
      );
    }
    if (ownerId) {
    ...
    const eventIds = [
      ...new Set((data || []).map((b: any) => b.event_id).filter(Boolean)),
    ];
  ...
  async getRevenue() {
    ...
    const [{ data: accounts }, { data: paidInvoices }] = await Promise.all([
      supabase
        .from('owner_accounts')
        .select('id, subscription_status, primary_owner_id'),
      supabase
        .from('invoices')
        .select('id, total_amount, status, created_at, owner_id')
        .eq('status', 'paid')
        .order('created_at', { ascending: false })
        .limit(50),
    ]);

    const activeSubscriptions = (accounts || []).filter((a: any) =>
      ['active', 'trialing'].includes(a.subscription_status),
    ).length;
    const cancelledSubscriptions = (accounts || []).filter(
      (a: any) => a.subscription_status === 'cancelled',
    ).length;
    ...
    (paidInvoices || []).forEach((inv: any) => {
      const key = new Date(inv.created_at).toLocaleDateString('en-US', {
        month: 'short',
        year: '2-digit',
      });
    ...
    const revenueByMonth = Object.entries(monthlyData).map(
      ([month, amount]) => ({ month, amount }),
    );

    const ownerIds = [
      ...new Set(
        (paidInvoices || []).map((i: any) => i.owner_id).filter(Boolean),
      ),
    ];
    const { data: owners } = ownerIds.length
      ? await supabase
  ...
  async getActivity(page = 1, limit = 100, search = '', roleFilter = '') {
    ...
    let query = supabase
      .from('users')
      .select(
        'id, email, first_name, last_name, role, last_sign_in_at, login_count, created_at',
        { count: 'exact' },
      )
  ...
  async getTrials(page = 1, limit = 50, search = '') {
    ...
    const {
      ...
      .from('owner_accounts')
      .select(
        'id, primary_owner_id, subscription_status, trial_ends_at, trial_days_used, created_at',
        { count: 'exact' },
      )
      .eq('subscription_status', 'trialing')
    ...
    let enriched = (accounts || []).map((a: any) => ({
      ...
      daysRemaining: a.trial_ends_at
        ? Math.max(
            0,
            Math.ceil((new Date(a.trial_ends_at).getTime() - now) / 86400000),
          )
        : null,
    }));
  ...
  async updateOwnerTrial(ownerId: string, action: string, days?: number) {
    ...
    const { data: account } = await supabase
      ...
      .eq('primary_owner_id', ownerId)
      .maybeSingle();
    const currentEnd = account?.trial_ends_at
  ...
  async updateTrialSettings(trialDays: number) {
    ...
    const { data, error } = await supabase
      .from('app_settings')
      .upsert(
        { key: 'FREE_TRIAL_DAYS', value: String(trialDays) },
        { onConflict: 'key' },
      )
      .select()
      .single();
  ...
  async getUnconvertedAccounts(
    ...
    let accountsQuery = supabase
      .from('owner_accounts')
      .select(
        'id, primary_owner_id, business_name, subscription_status, trial_ends_at, created_at',
        { count: 'exact' },
      )
    ...
    const { data: users } = ownerIds.length
      ? await supabase
          .from('users')
          .select(
            'id, email, first_name, last_name, last_sign_in_at, created_at',
          )
          .in('id', ownerIds)
      : { data: [] };
