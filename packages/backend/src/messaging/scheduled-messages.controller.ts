...
export class ScheduledMessagesController {
  ...
  async scheduleFromTemplate(
    @Body() data: { eventId: string; templateId: number },
  ): Promise<ScheduledMessage> {
    return this.scheduledMessagesService.scheduleFromTemplate(
      data.eventId,
      data.templateId,
    );
  }
  ...
  async scheduleMultipleFromTemplate(
    ...
    return this.scheduledMessagesService.scheduleMultipleFromTemplate(
      data.eventId,
      data.templateId,
    );
  }
