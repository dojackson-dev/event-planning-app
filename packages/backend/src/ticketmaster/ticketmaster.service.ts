...
export class TicketmasterService {
  ...
  private readonly baseUrl = 'https://app.ticketmaster.com/discovery/v2/events.json';
  ...
  async searchEvents(params: {
    ...
    if (params.zip_code) query.set('postalCode', params.zip_code);
    ...
    // Only future events
    query.set('startDateTime', new Date().toISOString().replace(/\.\d{3}Z$/, 'Z'));
    ...
    try {
      ...
      this.logger.log(`Ticketmaster: ${events.length} events for ${JSON.stringify(params)}`);
      return events;
    } catch (err) {
