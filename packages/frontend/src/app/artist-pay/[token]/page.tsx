...
function ArtistPayContent() {
  ...
  const [invoice, setInvoice] = useState<PublicArtistInvoice | null>(null)
  ...
  useEffect(() => {
    if (justPaid) {
      fetch(`${API_URL}/artist-invoices/public/${token}/verify-payment`, { method: 'POST' })
        .then(r => r.json())
  ...
  const fetchInvoice = async () => {
    try {
      const res = await fetch(`${API_URL}/artist-invoices/public/${token}`)
  ...
  const handlePay = async () => {
    ...
    try {
      const res = await fetch(`${API_URL}/artist-invoices/public/${token}/checkout`, {
        method: 'POST',
  ...
  // Platform fee is charged to the artist (deducted from payout), not shown to the client.
  // Stripe processing fee is passed through to the client.
  const invoiceBase = Number(invoice.subtotal) + Number(invoice.tax_amount) - Number(invoice.discount_amount)
  const processingFee = Math.round((Number(invoice.total_amount) - invoiceBase) * 100) / 100
      ...
      <div className="max-w-lg mx-auto">
        ...
        <div className="text-center mb-6">
          {artist?.profile_image_url ? (
            <img src={artist.profile_image_url} alt={displayName}
          ...
          {(artist?.city || artist?.state) && (
            <p className="text-xs text-gray-400">{[artist?.city, artist?.state].filter(Boolean).join(', ')}</p>
          )}
        </div>
        ...
        <div className="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden">
          ...
          <div className="px-6 py-4 border-b border-gray-100">
            ...
            <div className="mt-4 pt-4 border-t border-gray-100 space-y-1 text-sm">
              <div className="flex justify-between text-gray-500"><span>Subtotal</span><span>${Number(invoice.subtotal).toFixed(2)}</span></div>
              {Number(invoice.tax_rate) > 0 && (
                <div className="flex justify-between text-gray-500"><span>Tax ({invoice.tax_rate}%)</span><span>${Number(invoice.tax_amount).toFixed(2)}</span></div>
              )}
