...
export default function ArtistBookingLinkPage() {
  ...
  const fetchLink = async () => {
    try {
      const res = await api.get('/artists/booking-links/mine')
      if (res.data) {
        setLink(res.data)
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    setSaving(true)
    try {
      const res = await api.post('/artists/booking-links', {
  ...
  const bookingUrl = link ? `${window.location.origin}/book-artist/${link.slug}` : ''
  const shortUrl = link?.short_code ? `${window.location.origin}/a/${link.short_code}` : ''
  ...
  const copyShortLink = () => {
    ...
    setCopiedShort(true); setTimeout(() => setCopiedShort(false), 2000)
  }

  const suggestSlug = () => {
    setSlug(`artist-${Math.random().toString(36).substring(2, 8)}`)
  }

  return (
    <div className="min-h-screen bg-gray-50">
      <div className="max-w-2xl mx-auto px-4 py-8">
        <Link href="/artist/dashboard" className="text-sm text-gray-400 hover:text-gray-600 mb-2 inline-block">
          ← Artist Dashboard
        </Link>
          ...
          <form onSubmit={handleSave} className="space-y-5">
            <div className="bg-white rounded-xl border border-gray-200 p-6 shadow-sm">
              ...
              <p className="text-xs text-gray-400 mb-3">
                This becomes <span className="font-mono">/book-artist/<em>your-slug</em></span>.
                Lowercase letters, numbers, and hyphens only.
              </p>
              <div className="flex gap-2">
                <div className="flex-1 flex items-center border border-gray-200 rounded-lg overflow-hidden focus-within:ring-2 focus-within:ring-blue-500">
                  <span className="px-3 py-2 text-sm text-gray-400 bg-gray-50 border-r border-gray-200 whitespace-nowrap">/book-artist/</span>
                  <input
                    type="text"
                    value={slug}
            ...
            <div className="bg-white rounded-xl border border-gray-200 p-6 shadow-sm">
              ...
              <textarea
                ...
                placeholder="Hi! I'm available for events. Fill out the form and I'll be in touch soon."
            ...
            <div className="flex justify-end">
              <button
                type="submit"
                disabled={saving}
                className="inline-flex items-center gap-2 px-6 py-2 bg-blue-600 text-white rounded-lg text-sm font-medium hover:bg-blue-700 transition-colors disabled:opacity-60"
              >
