...
import { Calendar, User, Mail, Phone, MapPin, Clock, CheckCircle2, AlertCircle, Loader2, Megaphone, Send, ArrowRight } from 'lucide-react'
...
export default function PublicPromoterBookingPage() {
  ...
  useEffect(() => {
    const fetchLink = async () => {
      try {
        const res = await fetch(`${API_URL}/promoter/booking-link/${slug}`)
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setSubmitting(true)
    try {
      const res = await fetch(`${API_URL}/promoter/booking-link/${slug}/request`, {
        method: 'POST',
  ...
  if (loading) return <div className="min-h-screen bg-gray-50 flex items-center justify-center"><Loader2 className="w-8 h-8 animate-spin text-purple-600" /></div>
  ...
  const promoter = linkInfo.promoter_accounts
  const displayName = promoter.company_name || promoter.contact_name
    ...
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center px-4 py-10">
        <div className="bg-white rounded-2xl border border-gray-200 p-10 max-w-md w-full text-center shadow-sm">
          ...
          <p className="text-gray-500 text-sm mb-6">Your inquiry has been sent to <strong>{displayName}</strong>.</p>
          <button onClick={() => router.push('/')} className="inline-flex items-center gap-2 px-5 py-2.5 bg-purple-600 text-white rounded-xl text-sm font-medium hover:bg-purple-700">
      ...
      <div className="max-w-xl mx-auto">
        <div className="bg-white rounded-2xl border border-gray-200 shadow-sm p-6 mb-6 text-center">
          {promoter.profile_image_url ? (
            <img src={promoter.profile_image_url} alt={displayName} className="w-20 h-20 rounded-full object-cover mx-auto mb-4 border-4 border-purple-100 shadow" />
          ) : (
          ...
          <p className="text-sm text-gray-400 mt-0.5">Event Promoter</p>
          {promoter.location && <p className="text-xs text-gray-400 mt-1">{promoter.location}</p>}
          {promoter.bio && <p className="text-sm text-gray-500 mt-3 text-left">{promoter.bio}</p>}
        </div>

        {linkInfo.custom_message && (
          <div className="bg-purple-50 border border-purple-100 text-purple-800 rounded-xl p-4 mb-5 text-sm">{linkInfo.custom_message}</div>
        )}
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div>
              ...
              <label className="flex items-center gap-2 mt-2 cursor-pointer">
                <input type="checkbox" checked={smsOptIn} onChange={e => setSmsOptIn(e.target.checked)} className="w-4 h-4 rounded accent-purple-600" />
                <span className="text-xs text-gray-500">Send me text updates about this inquiry</span>
              </label>
            </div>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div className="sm:col-span-2">
                <label className="block text-xs font-medium text-gray-600 mb-1">Event Name</label>
                <input type="text" value={eventName} onChange={e => setEventName(e.target.value)} placeholder="e.g. Annual Festival, Club Night"
            ...
            <div>
              ...
              <textarea rows={3} value={notes} onChange={e => setNotes(e.target.value)} placeholder="Any additional details..."
            ...
            <button type="submit" disabled={submitting}
              className="w-full flex items-center justify-center gap-2 bg-purple-600 text-white font-semibold py-3 rounded-xl hover:bg-purple-700 transition-colors disabled:opacity-60 text-base">
              {submitting ? <><Loader2 className="w-5 h-5 animate-spin" /> Sending…</> : <><Send className="w-5 h-5" /> Send Inquiry</>}
            </button>
          </form>
        </div>
      </div>
