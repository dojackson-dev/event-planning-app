...
export default function OwnerBookingLinkPage() {
  ...
  const fetchLink = async () => {
    try {
      const res = await api.get('/owner/booking-links/mine')
      if (res.data) {
        setLink(res.data)
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    setSaving(true)
    try {
      const res = await api.post('/owner/booking-links', {
  ...
  const bookingUrl = link ? `${window.location.origin}/book-venue/${link.slug}` : ''
  const shortUrl = link?.short_code ? `${window.location.origin}/v/${link.short_code}` : ''
  ...
  const suggestSlug = () => setSlug(`venue-${Math.random().toString(36).substring(2, 8)}`)
                ...
                <div className="flex items-center justify-between gap-3">
                  <div className="flex items-center gap-2 min-w-0">
                    ...
                    <div className="min-w-0">
                      <p className="text-xs text-orange-400 mb-0.5">Short link</p>
                      <a href={shortUrl} target="_blank" rel="noreferrer" className="text-sm font-medium text-orange-700 hover:underline truncate block">{shortUrl}</a>
                    </div>
                  </div>
                  <button onClick={copyShortLink} className="flex-shrink-0 inline-flex items-center gap-1.5 text-xs font-medium px-3 py-1.5 rounded-lg bg-white border border-orange-200 text-orange-700 hover:bg-orange-50 transition-colors">
                    {copiedShort ? <CheckCircle2 className="w-3.5 h-3.5" /> : <Copy className="w-3.5 h-3.5" />}
                    {copiedShort ? 'Copied!' : 'Copy'}
                  </button>
                </div>
                <p className="text-xs text-orange-600/70 mt-1.5">Best for texting, printing, or QR codes — shorter and easier to share.</p>
              </div>
            )}
          </div>
        )}
          ...
          <form onSubmit={handleSave} className="space-y-5">
            <div className="bg-white rounded-xl border border-gray-200 p-6 shadow-sm">
              ...
              <p className="text-xs text-gray-400 mb-3">
                Becomes <span className="font-mono">/book-venue/<em>your-slug</em></span>. Lowercase letters, numbers, hyphens only.
              </p>
              <div className="flex gap-2">
                <div className="flex-1 flex items-center border border-gray-200 rounded-lg overflow-hidden focus-within:ring-2 focus-within:ring-orange-500">
                  <span className="px-3 py-2 text-sm text-gray-400 bg-gray-50 border-r border-gray-200 whitespace-nowrap">/book-venue/</span>
                  <input type="text" value={slug}
                    onChange={e => setSlug(e.target.value.toLowerCase().replace(/[^a-z0-9-]/g, ''))}
                    placeholder="my-venue-name" className="flex-1 px-3 py-2 text-sm focus:outline-none" required />
                </div>
            ...
            <div className="flex justify-end">
              <button type="submit" disabled={saving}
                className="inline-flex items-center gap-2 px-6 py-2 bg-orange-600 text-white rounded-lg text-sm font-medium hover:bg-orange-700 transition-colors disabled:opacity-60">
