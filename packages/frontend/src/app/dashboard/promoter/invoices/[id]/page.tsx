...
interface PromoterInvoice {
  ...
  promoter_accounts?: { company_name?: string; contact_name?: string; email: string }
}
...
export default function PromoterInvoiceDetailPage() {
  const { id } = useParams() as { id: string }
  const router = useRouter()

  const [invoice, setInvoice] = useState<PromoterInvoice | null>(null)
  ...
  useEffect(() => {
    api.get(`/promoter-invoices/${id}`)
      .then(r => setInvoice(r.data))
      .catch(e => setError(e.response?.data?.message || 'Invoice not found'))
      .finally(() => setLoading(false))
  }, [id])

  const payLink = invoice?.public_token ? `${FRONTEND_URL}/promoter-pay/${invoice.public_token}` : ''

  const handleSend = async () => {
    setSending(true); setError('')
    try {
      await api.post(`/promoter-invoices/${id}/send`)
      const r = await api.get(`/promoter-invoices/${id}`)
      setInvoice(r.data)
    } catch (e: any) {
  ...
  const handleDelete = async () => {
    ...
    setDeleting(true)
    try {
      await api.delete(`/promoter-invoices/${id}`)
      router.push('/dashboard/promoter/invoices')
    } catch (e: any) {
    ...
    <div className="bg-gray-50">
      ...
      <div className="bg-gradient-to-r from-purple-700 to-purple-500 px-4 py-6">
        <div className="max-w-3xl mx-auto flex items-center justify-between">
          ...
          <span className={`inline-flex items-center px-2.5 py-1 rounded-full text-xs font-medium ${STATUS_COLORS[invoice.status]}`}>
            {invoice.status}
          </span>
        </div>
      </div>
