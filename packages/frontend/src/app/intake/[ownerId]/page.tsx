...
export default function PublicIntakePage({ params }: { params: { ownerId: string } }) {
  ...
  useEffect(() => {
    fetch(`${API_URL}/intake-forms/public-form/${ownerId}`)
      .then(res => res.json())
      .then(data => {
        if (data.businessName) setBusinessName(data.businessName)
        if (data.logoUrl) setLogoUrl(data.logoUrl)
      })
      .catch(() => {/* non-fatal — form still works without owner name */})
  }, [ownerId])
    ...
    <div className="min-h-screen bg-gray-50 py-8 px-4">
      <div className="max-w-4xl mx-auto">
        ...
        <div className="text-center mb-8">
          {logoUrl && (
            <img src={logoUrl} alt={businessName} className="h-16 mx-auto mb-4 object-contain" />
          )}
          {businessName && (
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-4 sm:p-6 mb-6">
          ...
          <div className="flex justify-between mt-3">
            <span className={`text-xs sm:text-sm ${currentStep === 1 ? 'text-blue-600 font-semibold' : 'text-gray-500'}`}>Client</span>
            <span className={`text-xs sm:text-sm ${currentStep === 2 ? 'text-blue-600 font-semibold' : 'text-gray-500'}`}>Event</span>
            <span className={`text-xs sm:text-sm ${currentStep === 3 ? 'text-blue-600 font-semibold' : 'text-gray-500'}`}>Services</span>
            <span className={`text-xs sm:text-sm ${currentStep === 4 ? 'text-blue-600 font-semibold' : 'text-gray-500'}`}>Budget</span>
            <span className={`text-xs sm:text-sm ${currentStep === 5 ? 'text-blue-600 font-semibold' : 'text-gray-500'}`}>Review</span>
          </div>
        </div>
            ...
            <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-4 sm:p-6 space-y-6">
                  ...
                  {['phone', 'email', 'text'].map((method) => (
                    <label key={method} className="flex items-center">
                      <input type="radio" name="preferredContact" value={method}
                        checked={formData.preferredContact === method} onChange={handleChange} className="mr-2" />
              ...
              {formData.referralSource === 'referral' && (
                <div>
                  ...
                  <input type="text" name="referralDetails" value={formData.referralDetails} onChange={handleChange}
          ...
          {currentStep === 2 && (
            <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-4 sm:p-6 space-y-6">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">Preferred Event Date *</label>
                ...
                <div>
                  ...
                  <input type="number" name="estimatedGuests" required min="1" value={formData.estimatedGuests} onChange={handleChange}
                    className="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500" />
                </div>

                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">Confirmed Guests</label>
                  <input type="number" name="confirmedGuests" min="0" value={formData.confirmedGuests} onChange={handleChange}
                    className="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500" />
                </div>
              </div>

              <div>
                <label className="block text-sm font-medium text-gray-700 mb-2">Guest Age Range (select all that apply)</label>
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                  {['Children', 'Teens', 'Adults', 'Seniors'].map((age) => (
                    <label key={age} className="flex items-center">
                      <input type="checkbox" checked={formData.guestAgeRange.includes(age)}
                        onChange={() => handleCheckboxGroup('guestAgeRange', age)} className="mr-2" />
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">VIP Guests / Special Honorees</label>
                <textarea name="vipGuests" rows={2} value={formData.vipGuests} onChange={handleChange}
                ...
                <div className="flex flex-wrap gap-3">
                  {['indoor', 'outdoor', 'both'].map((venue) => (
                    <label key={venue} className="flex items-center">
                      <input type="radio" name="indoorOutdoor" value={venue}
                        checked={formData.indoorOutdoor === venue} onChange={handleChange} className="mr-2" />
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-2">Setup Style</label>
                <select name="setupStyle" value={formData.setupStyle} onChange={handleChange}
                  className="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <label className="flex items-center">
                  <input type="checkbox" name="needsDanceFloor" checked={formData.needsDanceFloor} onChange={handleChange} className="mr-2" />
                  <span className="text-sm">Need Dance Floor</span>
                </label>
                <label className="flex items-center">
                  <input type="checkbox" name="needsStage" checked={formData.needsStage} onChange={handleChange} className="mr-2" />
            ...
            <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-4 sm:p-6 space-y-6">
              ...
              <div className="border-b pb-6">
                <label className="flex items-center mb-3">
                  <input type="checkbox" name="needsCatering" checked={formData.needsCatering} onChange={handleChange} className="mr-3 h-5 w-5" />
                  <span className="text-base font-semibold">Catering Services</span>
                </label>
                {formData.needsCatering && (
                  <div className="ml-8 space-y-4">
                    <div>
                      <label className="block text-sm font-medium text-gray-700 mb-1">Catering Style</label>
                      <select name="cateringStyle" value={formData.cateringStyle} onChange={handleChange}
                        className="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500">
                    ...
                    <div>
                      <label className="block text-sm font-medium text-gray-700 mb-1">Dietary Restrictions / Special Requests</label>
                      <textarea name="dietaryRestrictions" rows={2} value={formData.dietaryRestrictions} onChange={handleChange}
              ...
              <div className="border-b pb-6">
                <label className="flex items-center mb-3">
                  <input type="checkbox" name="needsDecorator" checked={formData.needsDecorator} onChange={handleChange} className="mr-3 h-5 w-5" />
                ...
                {formData.needsDecorator && (
                  <div className="ml-8 space-y-4">
                    ...
                    <div>
                      <label className="block text-sm font-medium text-gray-700 mb-1">Decoration Style/Theme</label>
                      <input type="text" name="decorStyle" value={formData.decorStyle} onChange={handleChange}
                        placeholder="e.g., Rustic, elegant, modern, vintage"
                        className="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500" />
                    </div>
                    <div className="flex flex-wrap gap-4">
                      <label className="flex items-center">
                        <input type="checkbox" name="needsBalloons" checked={formData.needsBalloons} onChange={handleChange} className="mr-2" />
                        <span className="text-sm">Balloon Decorations</span>
                      </label>
                      <label className="flex items-center">
                        <input type="checkbox" name="needsMarquee" checked={formData.needsMarquee} onChange={handleChange} className="mr-2" />
              ...
              <div className="border-b pb-6">
                ...
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-2">Music/Entertainment Type</label>
                ...
                <div className="mt-4 flex flex-wrap gap-4">
                  <label className="flex items-center">
                    <input type="checkbox" name="needsPhotographer" checked={formData.needsPhotographer} onChange={handleChange} className="mr-2" />
                    <span className="text-sm">Photographer</span>
                  </label>
                  <label className="flex items-center">
                    <input type="checkbox" name="needsVideographer" checked={formData.needsVideographer} onChange={handleChange} className="mr-2" />
              ...
              <div className="border-b pb-6">
                ...
                <select name="barOption" value={formData.barOption} onChange={handleChange}
                  className="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500">
                ...
                {formData.barOption !== 'none' && (
                  <label className="flex items-center mt-3">
                    <input type="checkbox" name="beerWineOnly" checked={formData.beerWineOnly} onChange={handleChange} className="mr-2" />
                ...
                <div className="space-y-4">
                  <div>
                    <label className="flex items-center mb-2">
                      <input type="checkbox" name="needsSecurity" checked={formData.needsSecurity} onChange={handleChange} className="mr-3" />
                      <span className="text-sm font-medium">Security Personnel</span>
                    </label>
                    {formData.needsSecurity && (
                      <input type="number" name="securityCount" min="1" value={formData.securityCount} onChange={handleChange}
                  ...
                  <div>
                    <label className="flex items-center mb-2">
                      <input type="checkbox" name="needsParking" checked={formData.needsParking} onChange={handleChange} className="mr-3" />
                      <span className="text-sm font-medium">Parking Assistance</span>
                    </label>
                    {formData.needsParking && (
                      <input type="number" name="parkingSpaces" min="1" value={formData.parkingSpaces} onChange={handleChange}
                        placeholder="Estimated parking spaces needed"
                        className="ml-7 w-full sm:w-auto px-4 py-2 border border-gray-300 rounded-lg" />
                    )}
                  </div>
                  <label className="flex items-center">
                    <input type="checkbox" name="needsValet" checked={formData.needsValet} onChange={handleChange} className="mr-3" />
          ...
          {currentStep === 4 && (
            <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-4 sm:p-6 space-y-6">
              <div className="flex items-center mb-4">
                ...
                <h2 className="text-xl font-bold text-gray-900">Budget & Special Requests</h2>
              </div>

              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">Estimated Budget *</label>
                <select name="estimatedBudget" required value={formData.estimatedBudget} onChange={handleChange}
                  className="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500">
                ...
                <div className="flex flex-wrap gap-4">
                  {[
                    ...
                    <label key={value} className="flex items-center">
                      <input type="radio" name="budgetFlexibility" value={value}
                        checked={formData.budgetFlexibility === value} onChange={handleChange} className="mr-2" />
              ...
              <div>
                ...
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                  {[
                    ...
                    <label key={value} className="flex items-center">
                      <input type="radio" name="paymentPreference" value={value}
                        checked={formData.paymentPreference === value} onChange={handleChange} className="mr-2" />
                      <span className="text-sm">{label}</span>
                    </label>
                  ))}
                </div>
              ...
              <div className="border-t pt-6">
                ...
                <div className="space-y-4">
                  <div>
                    ...
                    <textarea name="specialRequests" rows={3} value={formData.specialRequests} onChange={handleChange}
                      placeholder="Any special requests, custom needs, or unique requirements..."
                      className="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500" />
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-1">Allergy Concerns</label>
                    <textarea name="allergies" rows={2} value={formData.allergies} onChange={handleChange}
                  ...
                  <div>
                    ...
                    <textarea name="accessibility" rows={2} value={formData.accessibility} onChange={handleChange}
                ...
                {formData.walkthroughRequested && (
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                      ...
                      <input type="date" name="preferredWalkthroughDate" value={formData.preferredWalkthroughDate}
                        onChange={handleChange}
                        className="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500" />
                    </div>
                    <div>
                      <label className="block text-sm font-medium text-gray-700 mb-1">Preferred Time</label>
                      <input type="time" name="preferredWalkthroughTime" value={formData.preferredWalkthroughTime}
                        onChange={handleChange}
                        className="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500" />
                    </div>
                  </div>
