...
interface LineItem {
  id?: string   // existing items have an id
  description: string
  quantity: number
  unit_price: number
}
...
export default function EditVendorInvoicePage() {
  ...
  useEffect(() => {
    const fetch = async () => {
      try {
        ...
        setNotes(inv.notes || '')
        setTerms(inv.terms || '')
        const lineItems: LineItem[] = (inv.vendor_invoice_items ?? []).map((i: any) => ({
          id: i.id,
          description: i.description,
          quantity: Number(i.quantity),
          unit_price: Number(i.unit_price),
        }))
        setItems(lineItems.length ? lineItems : [{ description: '', quantity: 1, unit_price: 0 }])
        setOriginalItemIds((inv.vendor_invoice_items ?? []).map((i: any) => i.id))
      } catch {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await api.put(`/vendor-invoices/${id}`, {
      ...
      await Promise.all(deletedIds.map(itemId => api.delete(`/vendor-invoices/items/${itemId}`)))

      // 3. Update existing items
      const existingItems = items.filter(i => i.id)
      await Promise.all(existingItems.map(i =>
        api.put(`/vendor-invoices/items/${i.id}`, {
          description: i.description,
          quantity: Number(i.quantity),
          unit_price: Number(i.unit_price),
        })
      ...
      await Promise.all(newItems.map(i =>
        api.post(`/vendor-invoices/${id}/items`, {
          description: i.description,
          quantity: Number(i.quantity),
          unit_price: Number(i.unit_price),
        })
      ...
      router.push(`/vendor-portal/invoices/${id}`)
    } catch (err: any) {
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      <div className="max-w-3xl mx-auto px-4 py-8">
        <Link href={`/vendor-portal/invoices/${id}`} className="text-sm text-gray-400 hover:text-gray-600 mb-1 inline-block">
          ← Back to Invoice
        </Link>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="flex justify-end gap-3">
            <Link
              href={`/vendor-portal/invoices/${id}`}
