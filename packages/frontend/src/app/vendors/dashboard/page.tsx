          ...
          <div className="bg-white rounded-xl shadow-sm p-6">
            ...
            {profile && (
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
                <div className="md:col-span-1 bg-gray-50 rounded-xl border border-gray-100 p-6 flex flex-col items-center text-center">
                  {profile.profile_image_url ? (
                    <img src={profile.profile_image_url} alt={profile.business_name}
                      className="w-20 h-20 rounded-full object-cover mb-3 border-4 border-primary-100" />
                  ) : (
                    <div className="w-20 h-20 rounded-full bg-primary-50 flex items-center justify-center mb-3 border-4 border-primary-100">
                      <User className="w-9 h-9 text-primary-400" />
                    </div>
                  )}
                  <h2 className="text-lg font-bold text-gray-900">{profile.business_name}</h2>
                  <p className="text-sm text-gray-500 mt-0.5">{CATEGORY_LABELS[profile.category] || profile.category}</p>
                  {profile.is_verified && (
                  ...
                  {profile.avgRating && (
                    <p className="mt-1 text-xs text-gray-400">⭐ {profile.avgRating} ({profile.reviewCount} reviews)</p>
                  )}
                </div>
                    ...
                    <div className="flex gap-4 flex-wrap">
                      {profile.hourly_rate > 0 && (
                        <div className="flex items-center gap-1.5 text-sm">
                          ...
                          <span className="font-semibold text-gray-900">${profile.hourly_rate}/hr</span>
                        </div>
                      )}
                      {profile.flat_rate > 0 && (
                        <div className="flex items-center gap-1.5 text-sm">
                          <DollarSign className="w-4 h-4 text-primary-500" />
                          <span className="font-semibold text-gray-900">${profile.flat_rate} flat rate</span>
                        </div>
                      )}
                      {profile.rate_description && (
                        <p className="text-xs text-gray-400 w-full mt-1">{profile.rate_description}</p>
                      )}
                    </div>
                  </div>
                  <div className="bg-gray-50 rounded-xl border border-gray-100 p-4">
                    ...
                    <div className="space-y-1.5">
                      ...
                      {profile.website && (
                        <div className="flex items-center gap-2 text-sm text-gray-700">
                          ...
                          <a href={profile.website} target="_blank" rel="noreferrer" className="text-primary-600 hover:underline">{profile.website}</a>
                        </div>
                      )}
                      {profile.instagram && (
                        <div className="flex items-center gap-2 text-sm text-gray-700">
                          <Instagram className="w-3.5 h-3.5 text-gray-400" /> @{profile.instagram.replace('@', '')}
                        </div>
                      )}
                    </div>
                  </div>
