...
export default function NewPromoterInvoiceScreen() {
  ...
  const handleSubmit = async () => {
    ...
    setSaving(true);
    try {
      const created = await apiRequest<{ id: string }>('/promoter-invoices', {
        method: 'POST',
        body: {
      ...
      router.replace(`/(promoter)/invoices/${created.id}` as any);
    } catch (err: any) {
...
const styles = StyleSheet.create({
  ...
  addItemText: { fontSize: 13, fontWeight: '600', color: Colors.purple },
  ...
  totalRow: {
    ...
    backgroundColor: Colors.surface, borderRadius: Radius.lg, padding: 16, marginBottom: 20, ...Shadow.sm,
  },
  totalLabel: { fontSize: 15, fontWeight: '700', color: Colors.textSecondary },
  totalValue: { fontSize: 20, fontWeight: '800', color: Colors.purple },
  submitBtn: {
    backgroundColor: Colors.purple, borderRadius: Radius.lg, paddingVertical: 14, alignItems: 'center',
  },
