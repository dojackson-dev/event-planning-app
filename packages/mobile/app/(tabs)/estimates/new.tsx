...
export default function NewEstimateScreen() {
  ...
  const handleCreate = async () => {
    ...
    try {
      ...
      const estimate = await apiRequest('/estimates', {
        method: 'POST',
        body: {
          estimate: {
            owner_id: user.id,
      ...
      Alert.alert('Estimate Created', 'Estimate saved as draft.', [
        { text: 'View', onPress: () => router.replace(`/(tabs)/estimates/${estimate.id}` as any) },
      ]);
    } catch (err: any) {
  ...
  return (
    <ScrollView style={styles.container} contentContainerStyle={styles.content} keyboardShouldPersistTaps="handled">
      ...
      <View style={styles.section}>
        ...
        <TextInput style={[styles.input, { marginTop: 8 }]} value={clientPhone} onChangeText={setClientPhone} placeholder="Phone (for SMS notification)" placeholderTextColor={Colors.textMuted} keyboardType="phone-pad" />
      </View>
      ...
      <View style={styles.section}>
        ...
        {lineItems.map((item, idx) => (
          <View key={item.id} style={styles.lineItem}>
            ...
            <TextInput style={styles.input} value={item.description} onChangeText={v => updateLine(item.id, 'description', v)} placeholder="Description" placeholderTextColor={Colors.textMuted} />
            <View style={styles.lineRow}>
              <View style={styles.lineField}>
                <Text style={styles.fieldLabel}>Qty</Text>
                <TextInput style={styles.smallInput} value={item.quantity} onChangeText={v => updateLine(item.id, 'quantity', v)} keyboardType="decimal-pad" placeholder="1" placeholderTextColor={Colors.textMuted} />
              </View>
              <View style={[styles.lineField, { flex: 2 }]}>
                <Text style={styles.fieldLabel}>Unit Price ($)</Text>
                <TextInput style={styles.smallInput} value={item.unit_price} onChangeText={v => updateLine(item.id, 'unit_price', v)} keyboardType="decimal-pad" placeholder="0.00" placeholderTextColor={Colors.textMuted} />
              </View>
              <View style={[styles.lineField, { flex: 2, alignItems: 'flex-end' }]}>
                <Text style={styles.fieldLabel}>Amount</Text>
                <Text style={styles.lineAmount}>{fmt((parseFloat(item.quantity) || 0) * (parseFloat(item.unit_price) || 0))}</Text>
              </View>
            </View>
          </View>
        ))}
      ...
      <View style={styles.section}>
        ...
        <View style={styles.card}>
          ...
          <View style={styles.totalRow}>
            ...
            <TextInput style={styles.inlineInput} value={globalDiscount} onChangeText={setGlobalDiscount} keyboardType="decimal-pad" placeholder="0" placeholderTextColor={Colors.textMuted} />
          </View>
          <View style={styles.totalRow}>
            <View style={styles.taxRow}><Text style={styles.totalLabel}>Tax</Text><Switch value={includeTax} onValueChange={setIncludeTax} trackColor={{ false: Colors.border, true: Colors.primary }} thumbColor="#FFF" style={{ transform: [{ scaleX: 0.8 }, { scaleY: 0.8 }] }} /></View>
            {includeTax && <View style={styles.taxInputRow}><TextInput style={[styles.inlineInput, { width: 60 }]} value={taxRate} onChangeText={setTaxRate} keyboardType="decimal-pad" placeholder="0" placeholderTextColor={Colors.textMuted} /><Text style={styles.totalLabel}>%  →  {fmt(taxAmount)}</Text></View>}
          </View>
          <View style={[styles.totalRow, styles.totalFinal]}><Text style={styles.totalFinalLabel}>Total</Text><Text style={styles.totalFinalValue}>{fmt(total)}</Text></View>
        </View>
      </View>
      ...
      <View style={styles.section}>
        ...
        <View style={styles.card}>
          <View style={styles.totalRow}><Text style={styles.totalLabel}>Issue Date</Text><TextInput style={styles.inlineInput} value={issueDate} onChangeText={setIssueDate} placeholder="YYYY-MM-DD" placeholderTextColor={Colors.textMuted} /></View>
          <View style={styles.totalRow}><Text style={styles.totalLabel}>Expiration Date</Text><TextInput style={styles.inlineInput} value={expirationDate} onChangeText={setExpirationDate} placeholder="YYYY-MM-DD" placeholderTextColor={Colors.textMuted} /></View>
        </View>
      </View>

      <View style={styles.section}>
        <Text style={styles.sectionLabel}>Notes</Text>
        <TextInput style={[styles.input, styles.multiline]} value={notes} onChangeText={setNotes} placeholder="Additional notes..." placeholderTextColor={Colors.textMuted} multiline numberOfLines={3} />
      </View>
      <View style={styles.section}>
        <Text style={styles.sectionLabel}>Terms</Text>
        <TextInput style={[styles.input, styles.multiline]} value={terms} onChangeText={setTerms} placeholder="Estimate terms..." placeholderTextColor={Colors.textMuted} multiline numberOfLines={3} />
      </View>

      <TouchableOpacity style={[styles.createBtn, saving && styles.createBtnDisabled]} onPress={handleCreate} disabled={saving}>
        {saving ? <ActivityIndicator color="#FFF" size="small" /> : (
          <><Ionicons name="document-text-outline" size={18} color="#FFF" /><Text style={styles.createBtnText}>Create Estimate</Text></>
        )}
      </TouchableOpacity>
    </ScrollView>
  );
...
const styles = StyleSheet.create({
  ...
  lineItem: { backgroundColor: Colors.surface, borderRadius: Radius.md, padding: 12, borderWidth: 1, borderColor: Colors.border, marginBottom: 8 },
  ...
  smallInput: { backgroundColor: Colors.background, borderRadius: Radius.sm, padding: 8, borderWidth: 1, borderColor: Colors.border, fontSize: 14, color: Colors.textPrimary },
  ...
  inlineInput: { backgroundColor: Colors.background, borderRadius: Radius.sm, paddingHorizontal: 10, paddingVertical: 6, borderWidth: 1, borderColor: Colors.border, fontSize: 14, color: Colors.textPrimary, minWidth: 80, textAlign: 'right' },
  ...
  createBtn: { backgroundColor: '#7C3AED', borderRadius: Radius.lg, padding: 16, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 8, marginTop: 8 },
