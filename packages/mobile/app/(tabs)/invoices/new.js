...
exports.default = NewInvoiceScreen;
...
function NewInvoiceScreen() {
    ...
    var _l = (0, react_1.useState)('Payment due within 30 days.'), terms = _l[0], setTerms = _l[1];
    var _m = (0, react_1.useState)(new Date().toISOString().slice(0, 10)), issueDate = _m[0], setIssueDate = _m[1];
    var _o = (0, react_1.useState)(new Date(Date.now() + 30 * 86400000).toISOString().slice(0, 10)), dueDate = _o[0], setDueDate = _o[1];
    ...
    var subtotal = lineItems.reduce(function (sum, l) {
    ...
    var handleCreate = function () { return __awaiter(_this, void 0, void 0, function () {
        var validItems, user, selectedEvent, items, invoice_1, err_1;
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0:
                ...
                case 2:
                    ...
                    items = validItems.map(function (item, idx) { return ({
                    ...
                    return [4 /*yield*/, (0, api_1.apiRequest)('/invoices', {
                            method: 'POST',
                            body: {
                                invoice: {
                                    owner_id: user.id,
                ...
                case 3:
                    invoice_1 = _a.sent();
                    react_native_1.Alert.alert('Invoice Created', 'Invoice saved as draft.', [
                        { text: 'View Invoice', onPress: function () { return router.replace("/(tabs)/invoices/".concat(invoice_1.id)); } },
                    ]);
                    return [3 /*break*/, 6];
                case 4:
                    err_1 = _a.sent();
                    react_native_1.Alert.alert('Error', err_1.message || 'Failed to create invoice');
                    return [3 /*break*/, 6];
                case 5:
                    setSaving(false);
                    return [7 /*endfinally*/];
    ...
    return (<react_native_1.ScrollView style={styles.container} contentContainerStyle={styles.content} keyboardShouldPersistTaps="handled">
      ...
      <react_native_1.View style={styles.section}>
        ...
        <react_native_1.TextInput style={[styles.input, { marginTop: 8 }]} value={clientPhone} onChangeText={setClientPhone} placeholder="Phone number (for SMS)" placeholderTextColor={theme_1.Colors.textMuted} keyboardType="phone-pad"/>
      </react_native_1.View>
        ...
        {lineItems.map(function (item, idx) { return (<react_native_1.View key={item.id} style={styles.lineItem}>
            ...
            <react_native_1.View style={styles.lineRow}>
              ...
              <react_native_1.View style={[styles.lineField, { flex: 2, alignItems: 'flex-end' }]}>
                ...
                <react_native_1.Text style={styles.lineAmount}>
      ...
      <react_native_1.View style={styles.section}>
        ...
        <react_native_1.View style={styles.card}>
          <react_native_1.View style={styles.totalRow}>
            ...
            <react_native_1.Text style={styles.totalValue}>{fmt(subtotal)}</react_native_1.Text>
          </react_native_1.View>
          <react_native_1.View style={styles.totalRow}>
          ...
          <react_native_1.View style={styles.totalRow}>
            <react_native_1.View style={styles.taxRow}>
              <react_native_1.Text style={styles.totalLabel}>Tax</react_native_1.Text>
              <react_native_1.Switch value={includeTax} onValueChange={setIncludeTax} trackColor={{ false: theme_1.Colors.border, true: theme_1.Colors.primary }} thumbColor="#FFF" style={{ transform: [{ scaleX: 0.8 }, { scaleY: 0.8 }] }}/>
            </react_native_1.View>
            {includeTax && (<react_native_1.View style={styles.taxInputRow}>
                <react_native_1.TextInput style={[styles.inlineInput, { width: 60 }]} value={taxRate} onChangeText={setTaxRate} keyboardType="decimal-pad" placeholder="0" placeholderTextColor={theme_1.Colors.textMuted}/>
                <react_native_1.Text style={styles.totalLabel}>%  →  {fmt(taxAmount)}</react_native_1.Text>
              </react_native_1.View>)}
          </react_native_1.View>
          <react_native_1.View style={[styles.totalRow, styles.totalFinal]}>
            <react_native_1.Text style={styles.totalFinalLabel}>Total</react_native_1.Text>
            <react_native_1.Text style={styles.totalFinalValue}>{fmt(total)}</react_native_1.Text>
          </react_native_1.View>
        </react_native_1.View>
      </react_native_1.View>
      ...
      <react_native_1.View style={styles.section}>
        ...
        <react_native_1.View style={styles.card}>
          <react_native_1.View style={styles.totalRow}>
            <react_native_1.Text style={styles.totalLabel}>Issue Date</react_native_1.Text>
            <react_native_1.TextInput style={styles.inlineInput} value={issueDate} onChangeText={setIssueDate} placeholder="YYYY-MM-DD" placeholderTextColor={theme_1.Colors.textMuted}/>
          </react_native_1.View>
          <react_native_1.View style={styles.totalRow}>
            <react_native_1.Text style={styles.totalLabel}>Due Date</react_native_1.Text>
            <react_native_1.TextInput style={styles.inlineInput} value={dueDate} onChangeText={setDueDate} placeholder="YYYY-MM-DD" placeholderTextColor={theme_1.Colors.textMuted}/>
          </react_native_1.View>
        </react_native_1.View>
      </react_native_1.View>
      ...
      <react_native_1.View style={styles.section}>
        ...
        <react_native_1.TextInput style={[styles.input, styles.multiline]} value={terms} onChangeText={setTerms} placeholder="Payment terms..." placeholderTextColor={theme_1.Colors.textMuted} multiline numberOfLines={3}/>
      </react_native_1.View>
      ...
      <react_native_1.TouchableOpacity style={[styles.createBtn, saving && styles.createBtnDisabled]} onPress={handleCreate} disabled={saving}>
        {saving ? <react_native_1.ActivityIndicator color="#FFF" size="small"/> : (<>
            <vector_icons_1.Ionicons name="receipt-outline" size={18} color="#FFF"/>
            <react_native_1.Text style={styles.createBtnText}>Create Invoice</react_native_1.Text>
          </>)}
      </react_native_1.TouchableOpacity>
...
var styles = react_native_1.StyleSheet.create({
    ...
    selectBox: {
        backgroundColor: theme_1.Colors.surface, borderRadius: theme_1.Radius.md, padding: 14,
        borderWidth: 1, borderColor: theme_1.Colors.border, flexDirection: 'row',
        justifyContent: 'space-between', alignItems: 'center',
    },
    selectText: { fontSize: 15, color: theme_1.Colors.textPrimary, flex: 1 },
    placeholder: { color: theme_1.Colors.textMuted },
    dropdown: __assign({ backgroundColor: theme_1.Colors.surface, borderRadius: theme_1.Radius.md, marginTop: 4, borderWidth: 1, borderColor: theme_1.Colors.border, maxHeight: 220, overflow: 'scroll' }, theme_1.Shadow.md),
    ...
    input: {
        backgroundColor: theme_1.Colors.surface, borderRadius: theme_1.Radius.md, padding: 12,
        borderWidth: 1, borderColor: theme_1.Colors.border, fontSize: 15, color: theme_1.Colors.textPrimary,
    },
    multiline: { height: 80, textAlignVertical: 'top' },
    lineItem: {
        backgroundColor: theme_1.Colors.surface, borderRadius: theme_1.Radius.md, padding: 12,
        borderWidth: 1, borderColor: theme_1.Colors.border, marginBottom: 8,
    },
    ...
    smallInput: {
        backgroundColor: theme_1.Colors.background, borderRadius: theme_1.Radius.sm, padding: 8,
        borderWidth: 1, borderColor: theme_1.Colors.border, fontSize: 14, color: theme_1.Colors.textPrimary,
    },
    ...
    inlineInput: {
        backgroundColor: theme_1.Colors.background, borderRadius: theme_1.Radius.sm, paddingHorizontal: 10,
        paddingVertical: 6, borderWidth: 1, borderColor: theme_1.Colors.border, fontSize: 14,
        color: theme_1.Colors.textPrimary, minWidth: 80, textAlign: 'right',
    },
    ...
    createBtn: {
        backgroundColor: theme_1.Colors.primary, borderRadius: theme_1.Radius.lg, padding: 16,
