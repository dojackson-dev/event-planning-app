...
export default function VendorContractsScreen() {
  ...
  const fetchContracts = useCallback(async () => {
    try {
      setError('');
      const data = await apiRequest<Contract[]>('/contracts/vendor/mine');
      ...
      <FlatList
        ...
        ListEmptyComponent={
          <EmptyState
            ...
            message={error || 'Contracts sent to you by venues will show up here.'}
          />
        }
        renderItem={({ item }) => {
          ...
          return (
            <TouchableOpacity
              ...
              onPress={() => router.push(`/(vendor)/contracts/${item.id}` as any)}
            >
