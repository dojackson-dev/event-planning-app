...
export default function VendorInvoiceDetailScreen() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      setError('');
      const data = await apiRequest<VendorInvoiceDetail>(`/vendor-invoices/${id}`);
      setInvoice(data);
    } catch (err: any) {
  ...
  const handleSend = async () => {
    ...
    setSending(true);
    try {
      await apiRequest(`/vendor-invoices/${invoice.id}/send`, { method: 'POST' });
      ...
      <View style={styles.section}>
        ...
        <View style={styles.card}>
          ...
          <View style={[styles.infoRow, { borderBottomWidth: 0 }]}>
            ...
            <Text style={[styles.infoValue, { color: Colors.primary }]}>${Number(invoice.amount_due).toFixed(2)}</Text>
          </View>
        </View>
      </View>
...
const styles = StyleSheet.create({
  ...
  invoiceNum: { fontSize: 16, fontWeight: '700', color: Colors.primary },
  ...
  sendBtn: {
    ...
    backgroundColor: Colors.primary, borderRadius: Radius.lg, paddingVertical: 14, marginTop: 8,
  },
