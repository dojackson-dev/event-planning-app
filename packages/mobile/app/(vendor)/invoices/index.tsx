...
export default function VendorInvoicesScreen() {
  ...
  const load = useCallback(async () => {
    try {
      setError('');
      const data = await apiRequest<VendorInvoice[]>('/vendor-invoices/mine');
      setInvoices(Array.isArray(data) ? data : []);
    } catch (err: any) {
      setError(err.message || 'Failed to load invoices. Make sure your vendor account is set up.');
    } finally {
      setLoading(false);
      setRefreshing(false);
    }
  ...
  return (
    <View style={styles.container}>
      <View style={styles.header}>
        ...
        <TouchableOpacity style={styles.newBtn} onPress={() => router.push('/(vendor)/invoices/new' as any)}>
      ...
      <FlatList
        ...
        renderItem={({ item }) => {
          ...
          return (
            <TouchableOpacity
              style={styles.card}
              onPress={() => router.push(`/(vendor)/invoices/${item.id}` as any)}
              activeOpacity={0.75}
            >
        ...
        ListEmptyComponent={
          <View style={styles.empty}>
            ...
            <TouchableOpacity style={styles.emptyBtn} onPress={() => router.push('/(vendor)/invoices/new' as any)}>
