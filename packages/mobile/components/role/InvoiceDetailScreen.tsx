...
function extractItems(inv: InvoiceDetail): InvoiceItem[] {
  return (
    inv.items ||
    inv.vendor_invoice_items ||
    inv.artist_invoice_items ||
    inv.promoter_invoice_items ||
    []
  );
}
...
export default function InvoiceDetailScreen({ apiBase }: Props) {
  ...
  const load = useCallback(async () => {
    if (!id) return;
    try {
      const data = await apiRequest<InvoiceDetail>(`${apiBase}/${id}`);
      setInvoice(data);
    } catch (err: any) {
  ...
  const sendInvoice = () => {
    Alert.alert('Send Invoice', 'This will email the invoice to the client. Continue?', [
      { text: 'Cancel', style: 'cancel' },
      {
        text: 'Send', onPress: async () => {
          setActioning(true);
          try {
            await apiRequest(`${apiBase}/${id}/send`, { method: 'POST' });
            await load();
            Alert.alert('Sent', 'Invoice sent to client.');
          } catch (err: any) {
    ...
    Alert.alert('Mark as Paid', 'Mark this invoice as fully paid?', [
      { text: 'Cancel', style: 'cancel' },
      {
        text: 'Mark Paid', onPress: async () => {
          setActioning(true);
          try {
            await apiRequest(`${apiBase}/${id}`, { method: 'PUT', body: { status: 'paid' } });
            await load();
          } catch (err: any) {
            Alert.alert('Error', err.message);
          } finally {
      ...
      <ScrollView style={styles.container} contentContainerStyle={styles.content}>
        <View style={styles.headerCard}>
          <View style={styles.headerTop}>
            <View>
              ...
              {invoice.client_name && <Text style={styles.clientName}>{invoice.client_name}</Text>}
            </View>
        ...
        {items.length > 0 && (
          <View style={styles.section}>
            ...
            <View style={styles.card}>
              {items.map((item, idx) => (
                <View key={item.id} style={[styles.lineRow, idx < items.length - 1 && styles.lineBorder]}>
                  <View style={{ flex: 1 }}>
